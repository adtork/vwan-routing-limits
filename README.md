...
> **💡 Cross-path lever — disjoint prefixes / LPM steering across ExR, VPN, and SD-WAN:** Across the three on-prem paths (ExpressRoute, S2S VPN, SD-WAN NVA), you can also offload one gateway by advertising **disjoint** prefix sets per path, or by using **longest-prefix match (LPM)** to shift specific destinations onto a lighter-loaded GW. For example, advertise **more-specific** prefixes over SD-WAN or VPN so those destinations are pulled off the ExpressRoute GW (relieving ④ 9,500 inbound on the ER GW), while keeping the supernet over ExR as a fallback. This trades the load between ④ (ER GW inbound), ⑦ (VPN GW 4k aggregate), ⑨/⑩ (NVA peers + per-peer share of ①), and ① (10k hub ceiling) — pick the GW with the most headroom for each prefix block. *Note: ③ (1k ER GW → MSEE outbound) is unaffected — that cap is about Azure prefixes leaving the hub, not on-prem prefixes entering it.*
